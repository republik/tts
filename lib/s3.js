...
export const createProductionUploader =
  (storage) => async (documentId, production) => {
    const debug = _debug('s3:production:upload')
    const { asset } = production

    if (!asset) {
      debug('audio asset missing', production)
      return
    }

    const hash = createHash(production)

    /* org, name, commitId, versioName */
    const [org, name] = Buffer.from(documentId, 'base64')
      .toString('utf-8')
      .split('/')
    const Key = ['tts', org, name, 'published', `${hash}.mp3`].join('/')
    ...
    debug('done. %o', { ...production, asset: !!asset })

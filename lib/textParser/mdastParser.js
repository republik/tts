...
const voice = {
  match: ({ type, data }) => type === 'span' && data?.type === 'VOICE',
  tokenize: (node, path) => {
    // we need to tag this text with another voice
    if (node.data?.syntheticVoice) {
      return `{{${node.data?.syntheticVoice}||${walk(node, path)}}}`
    }

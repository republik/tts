...
const listItemOrdered = {
  ...
  tokenize: (node, path) => {
    ...
    return [
      ...
      ...nodes.slice(1, -1).map((n) => ({ ...n, type: 'list' })),
    ]
  },
}
...
const datawrapper = {
  match: ({ type, identifier }) =>
    type === 'zone' && identifier === 'EMBEDDATAWRAPPER',
  tokenize: unspeakable,
}

const story = {
  match: ({ type, identifier }) =>
    type === 'zone' && identifier === 'STORYCOMPONENT',
  tokenize: unspeakable,
}
...
const webOnly = {
  match: ({ type, identifier }) => type === 'zone' && identifier === 'WEBONLY',
  tokenize: (node, path) => walk(node, path).flat(),
}

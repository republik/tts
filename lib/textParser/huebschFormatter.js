...
export const formatForHuebsch = (defaultVoice) => (node) => {
  const { text, preface, type, caesura, voice } = node

  const paragraph = formatParagraph(voice || defaultVoice, text, preface)

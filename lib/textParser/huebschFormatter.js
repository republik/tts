...
export const addTtsNotice = (ogText) => {
  if (!ogText) {
    return { text: TTS_NOTICE, authors: [] }
  }
  const debug = _debug('speakableText:addTtsNotice')
  try {
    const authorsText = getAuthors(ogText)
    ...
    const ttsText = `Ein Beitrag von ${makeCommaSeparatedString(
      authors,
...
const formatCredits = (paragraph) => {
  const { text, authors } = addTtsNotice(paragraph.content[0].text)
  return [
    {
      ...paragraph,
      content: [
...
const formatParagraph = (voice, text, preface, meta) => {
...
export const formatForHuebsch = (defaultVoice, secondVoice) => (node) => {
  ...
  const paragraph = formatParagraph(
    ...
    text,
    preface,
    { role: type }, // changing from type to role, as type is used to describe the huebsch block type as well
  )

  return type === 'subtitle'
    ? withPause(paragraph, 1.4, 1.4)
    : type === 'credits'

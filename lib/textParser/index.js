...
const addCreditsNode = (textNodes) => {
  if (textNodes[2]?.type === 'credits') return textNodes
  // credits is the third node. lead is first, title second.
  // but if there are no credits, we add the note as the first node,
  // as the main use case for this is newsletters, which don't have a lead or title.
  return [
    {
      type: 'credits',
    },
...
export const getSpeakableText = (content) => {
  ...
  const voice2 = content.meta.syntheticVoice2
  if (!voice) {
  ...
  try {
    ...
    const cleanText = filterTokens(tokens).map(removeEllipses).map(trimText)
    const speakableText = addCreditsNode(cleanText).map(
      formatForHuebsch(voice, voice2),
    )

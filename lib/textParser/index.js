...
// we use (abuse?) the memo to tag a section with a specific voice
// the tagged text follows this pattern:
// {{voice:tagged text}}
// this function extracts the tagged text
const tagWithVoice = (token) => {
  const text = token.text
  if (!token.text) return token
  const taggedRe = /{{(.+)\|\|(.+)}}/
  const tagged = text.match(taggedRe)
  if (tagged) {
    return [
      { ...token, text: text.split('{{')[0] },
      { ...token, text: tagged[2], voice: tagged[1] },
      { ...token, text: text.split('}}')[1] },
    ]
  }
  return token
}
...
export const getSpeakableText = (content) => {
  ...
  try {
    ...
    const cleanText = filterTokens(tokens.map(tagWithVoice).flat(1))
      .map(removeEllipses)
      .map(trimText)

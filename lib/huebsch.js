...
export const getFromHuebsch = async (body) => {
  ...
  const audioFile = await fetchWithRetry(audioUrl)
    .then((res) => {
      debug('res %o', res)
      if (!res.ok) {
        return false
...
const fetchWithRetry = async (url, retries = 3, waitForMs = 1000) => {
  const debug = _debug('huebsch:fetchWithRetry')
  let error
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      const res = await fetch(url)
      if (!res.ok) {
        throw new Error('Response not ok, retrying...')
      }
      return res
    } catch (e) {
      debug(
  ...
  throw new HuebschError(`failed to fetch ${retries} times with error`, error)
}

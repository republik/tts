...
export const render = async (speakables, meta, voice = 'alpha') =>
  [preface, ...speakables]
    .filter((speakable) => !speakable.unspeakable)
    .filter((speakable) => !!speakable.text)
    .map((speakable) => {
      ...
      switch (type) {
        case 'preface':
          return {
            ...speakable,
            node: `<voice name="${name}"><prosody rate="${rate}" pitch="${pitch}st">${text}</prosody><break time="1.5s" /></voice>`,
          }
        case 'lead':
          return {
            ...speakable,
            node: `<voice name="${name}"><prosody rate="${rate}" pitch="${pitch}st">${text}</prosody><break time="1s" /></voice>`,
          }
        case 'title': {
          ...
          return {
            ...speakable,
            node: `<voice name="${name}"><prosody rate="${reducedRate}" pitch="${pitch}st">${title}</prosody></voice>`,
          }
        }
        case 'credits':
          return {
            ...speakable,
            node: `<voice name="${name}"><prosody rate="${rate}" pitch="${pitch}st">${text}</prosody><break time="2s" /></voice>`,
          }
        case 'subtitle': {
          const reducedRate = (rate * 10 - 1) / 10
          return {
            ...speakable,
            node: `<voice name="${name}"><prosody rate="${reducedRate}" pitch="${pitch}st">${text}</prosody></voice>`,
          }
        }
        // @TODO: Parse credits a little better?
        // @TODO: Prepend "This is a synthesized voice."
        default:
          return {
            ...speakable,
            node: `<voice name="${name}"><prosody rate="${rate}" pitch="${pitch}st">${text}</prosody></voice>`,
          }
      }
    })
    .map((speakable) => {
...
export const toSpeech = (ssml, voice = 'alpha') =>
  new Promise((resolve, reject) => {
    ...
    synthesizer.SynthesisCanceled = function (s, e) {
      ...
      let str =
        'synthesis cancelled: ' + CancellationReason[cancellationDetails.reason]
